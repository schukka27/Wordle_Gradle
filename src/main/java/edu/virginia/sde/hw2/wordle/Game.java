...
public class Game {
    ...
    public GuessResult submitGuess(String guess) {
        GuessResult newGuess = new GuessResult(guess, getAnswer());
        if (isGameOver()) {
            throw new GameAlreadyOverException("The game is already over!");
        }
        //guess isn't in dictionary then throw exception
        else if (!guessDictionary.contains(guess)) {
            throw new IllegalWordException("Your guess is not valid");
        }
        //if the guess is wrong then decrement guesses remaining; if guesses remaining is 0 after decrementing then set gameStatus to loss
        ...
        //if the guess is correct still decrement guesses remaning but set gameStatus to win
        else if (newGuess.isCorrect()) {
            guessesRemaining = guessesRemaining - 1;
            gameStatus = WIN;
        }
