...
public class GuessResult {
    ...
    public LetterResult[] getLetterResults() {
        //TODO: Stub
        String tempguess = guess.toLowerCase();
        String tempanswer= answer.toLowerCase();
        LetterResult [] letterResult = new LetterResult [tempguess.length()];
        boolean [] guessboolean = new boolean[tempguess.length()];
        boolean [] answerboolean = new boolean[tempanswer.length()];
        for(int i=0; i<=4; i++) {
            if (tempguess.substring(i, i + 1).equals(tempanswer.substring(i, i + 1))) {
                letterResult[i] = LetterResult.GREEN;
                guessboolean[i] = true;
                answerboolean[i] = true;
            }

        }
        for (int j = 0; j < tempguess.length(); j++){
            for (int k = 0; k < tempanswer.length();k++){
                if (letterResult[j] == null && tempguess.charAt(j) == tempanswer.charAt(k) && !guessboolean[j] && !answerboolean[k]) {
                    letterResult[j] = LetterResult.YELLOW;
                    guessboolean[j] = true;
                    answerboolean[k] = true;
                    break;
                }
            }
        }
        for (int p = 0; p<tempguess.length();p++){
            if (letterResult[p] == null){
                letterResult[p] = LetterResult.GRAY;
            }
        }
